...
export default function TextConv(props) {
  ...
  return (
    ...
    <div className="form-group">
      <h1 htmlFor="uname">Text Converter:</h1>     
     <textarea name="inputbox" id="inputbox" cols="10" rows="5" className="form-control" value={text} onChange={ChangeValue}></textarea>
    </div>
    <div className="form-group">
      <button type="button" className="btn btn-primary mr-4" id="conv-upper-lower" onClick={convertToUpper}>{capText}</button>
      <button type="button" className="btn btn-danger" onClick={clearText}>Clear textArea</button>
      <button type="button" className="btn btn-info ml-3" onClick={handleCopy}>copy text</button>
      <button type="button" className="btn btn-primary ml-3" onClick={handleExtraSpaces}>Remove Space</button>
    </div>
    </div>
    <div className="container mb-3">
        <p>{text === "" ? "0" : text.split(" ").length} word and {text.length} characters</p>
    </div>
    <div className="container mb-3">
        <h3>Preview</h3>
